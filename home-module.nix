...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    # ── Element: patch app.asar to inject ~/.config/chromashell/element.css ──
    # Appends a browser-window-created listener to electron-main.js inside the
    # asar so Element loads the CSS generated by caelestia-posthook.sh on every
    # page load. Uses app/fs/path which are already imported in electron-main.js.
    nixpkgs.overlays = lib.optional (cfg.comms.app == "element") (_final: prev: {
